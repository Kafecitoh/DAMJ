...
public class Meiga {
    ...
    public Meiga(String nome, String alcume, Collection<Feitizo> feitizos, Map<String, Integer> inventario) {
        ...
        this.inventario = inventario = new TreeMap<>();

    }

    public int lanzarFeitizo(){
        List<Feitizo> ordenados = new ArrayList <>(feitizos);
            ordenados.sort((a,b) -> {
                int diff = b.getDificultado().compareTo(a.getDificultado());
                    return diff != 0 ? diff : a.getNome().compareToIgnoreCase(b.getNome));
            });
    }
